...
const validateCropDimensions = (
	...
	if (width <= 0 || height <= 0 || left < 0 || top < 0) return false;
...
const validateResizeDimensions = (width: number, height: number): boolean => {
	if (width <= 0 || width > 10000 || height <= 0 || height > 10000)
		//preventing invalid dimensions and setting a limit for image sizes to limit resources usage.
		return false;
	return true;
};
...
const cropImage = async (
	...
	const validDimensions = validateCropDimensions(
		width,
		height,
		userWidth,
		userHeight,
	...
	if (!validDimensions) throw new APIError("Invalid crop dimensions.", 400);
	await sharp(imagePath)
		.extract({ width: userWidth, height: userHeight, left, top })
		.toFile(path.join("./public", newName));
};
...
const resizeImage = async (
	...
	if (!validDimensions) throw new APIError("Invalid resize dimensions.", 400);
	if (respectAspectRatio) {
		...
		const { width: newWidth, height: newHeight } =
			await sharp(newFilePath).metadata();
...
const blurImage = async (imagePath: string, newName: string, sigma: number) => {
	if (sigma < 0.3 || sigma > 100)
		// limit of 300 to limit resource usage.
