...
const supportedFile = (fileName: string): boolean => {
	// a simple function to check if the file type is supported.
...
const storage = multer.diskStorage({
	...
	filename: function (req, file, cb) {
		if (!supportedFile(path.extname(file.originalname)))
			// checking for file type, if it's not supported, return an error.
...
const uploadFile = (req: Request, res: Response, next: NextFunction): void => {
	upload.single("image")(req, res, (err) => {
		...
		if (!req.file) return next(new APIError("no file provided", 400));
		else next();
	});
};
